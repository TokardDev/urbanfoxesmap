...
def limit_request_rate(max_requests, period):
    """
    Decorator to limit the request rate to a certain number of requests per period.
    """
    request_times = []

    def decorator(func):
        def wrapper(*args, **kwargs):
            # Remove all request times that are older than the period
            request_times[:] = [t for t in request_times if t > time.monotonic() - period]
            print(request_times)

            # If the request rate is too high, raise an exception
            if len(request_times) >= max_requests:
                return make_response(f"Too many requests. Limit is {max_requests} requests per {period} seconds.", 429)

            # Call the function and record the request time
